...
export default class PaymentMethodClient1 extends Component {
    ...
    createNewStripeUser(token) {
        ...
        fetch(constants.ClientNewStripeUser, {
            method: 'POST',
            headers: {
            ...
            .then(response => {
                console.log("responsecreateNewStripeUser-->", "-" + JSON.stringify(response));
                if (response.ResultType === 1) {
                    ...
                    this.setState({showLoading: false});
                    Alert.alert("Error!", "Could not created stripe account please try again")
    ...
    doPaymentFlow()
    {
        if(selected_surge_price===true)
        {
            total_price=parseInt(total_price)*1.5
        }
        let details = {
            barberID: barber_id,
        ...
        formBody = formBody.join("&");
        fetch(constants.ClientStripePaymentFlow, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/x-www-form-urlencoded'
            ...
            .then(response => {
                console.log("responseClientStripePaymentFlow-->", "-" + JSON.stringify(response));
                if (response.ResultType === 1) {
                    cus_stripe_id=response.Data.cus_stripe_id;
                    transaction_id=response.Data.transaction_id;
                    balance_transaction=response.Data.balance_transaction;
                    destination=response.Data.destination;
                    destination_payment=response.Data.destination_payment;
                    source_transaction=response.Data.source_transaction;
                    stripedate=response.Data.date;
                    created=response.Data.created;

                    this.bookApointment();
                } else {
                    this.setState({showLoading: false});
                    Alert.alert("Error!", "Could not do stripe Payment please try again")
                    if (response.ResultType === 0) {
                        alert(response.Message);
                    }
                }
    ...
    saveCard = async (shouldPass = true) => {
        ...
        try {
            ...
            if(token.tokenId)
            {
                this.createNewStripeUser(token.tokenId);
                        ...
                        <View style={{marginStart: 10, marginEnd: 10}}>
                            <Text style={styles.txtHeader}>CARD NUMBER</Text>
                                ...
                                <View style={[globalStyles.rowBackground, {flex: 1, flexDirection: 'row', height: 40,}]}>
                                    ...
                                    <TextInput style={{fontSize: 15, color: 'white', marginStart: 10,width:"70%"}}
                            ...
                            <View style={{width: "100%", flexDirection: "row"}}>
                                ...
                                <View style={[styles.row_back, {width: "25%"}]}>
                                    <TextInput style={{fontSize: 15, color: 'white', marginStart: 5,width:"100%"}}
                                               placeholder={"123"}
                            ...
                            <View style={{flex: 1, flexDirection: 'column', width: "100%"}}>
                                <View style={{flexDirection: "row", alignItems: "center"}}>
                                    <TextInput
                                        style={{height: 40, color: "white", marginStart: 30,width:"80%"}}
